...
async function connectDatabase() {
  const configuredUri = process.env.MONGODB_URI ?? "mongodb://127.0.0.1:27017/verdict";

  try {
    await mongoose.connect(configuredUri);
    return configuredUri;
  } catch (error) {
    console.warn("[mongo] primary MongoDB not available, starting in-memory fallback", {
      configuredUri,
    ...
    console.log("[mongo] using in-memory MongoDB at", fallbackUri);
    return fallbackUri;
  }
}
